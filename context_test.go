...
import (
	...
	testdata "github.com/pckhoi/gin/testdata/protoexample"
...
func TestContextHandlerName(t *testing.T) {
	...
	assert.Regexp(t, "^(.*/vendor/)?github.com/pckhoi/gin.handlerNameTest$", c.HandlerName())
}
...
func TestContextHandlerNames(t *testing.T) {
	...
	for _, name := range names {
		assert.Regexp(t, `^(.*/vendor/)?(github\.com/pckhoi/gin\.){1}(TestContextHandlerNames\.func.*){0,1}(handlerNameTest.*){0,1}`, name)
	}
}
...
func TestContextInitQueryCache(t *testing.T) {
	validURL, err := url.Parse("https://github.com/pckhoi/gin/pull/3969?key=value&otherkey=othervalue")
	require.NoError(t, err)
